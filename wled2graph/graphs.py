...
from wled2graph.template import css_table, css_template
...
def make_document(doc, args):
    div_head = Div(
        text=(
            ...
            '<a href="https://github.com/bigredfrog/wled2graph" target="_blank" style="color: inherit;">'
            "wled2graph</a></h1>"
        )
    )
    ...
    plot_ping = figure(
        title="Real-time update for ping",
        x_axis_label="time (s)",
    ...
    plot_rssi = figure(
        title="Real-time update for rssi",
        x_axis_label="time (s)",
        y_axis_label="rssi",
        width=1500,
        height=graph_height,
    ...
    custom_tooltip = """
        <div style="background: black; margin:-10px; padding: 10px; border-radius: 10px; border: 1px solid white;
        ...
        with data_lock:
            ...
            for idx, ip in enumerate(data_source.keys()):
                ...
                if data_source[ip]["name"] is None:
                    new_table_html += (
                        f"<tr>{td_style}{ip}</td><td>Not Found</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>"
                    )
                else:
                    new_table_html += (
...
def run_bokeh_app(args):
    ...
    _LOGGER.info(f"Serving Bokeh app on http://{host_ip}:{server_settings['port']}/")
